...
if __name__ == '__main__':
    ...
    parser.add_argument('--wine', action='store_true', help='Run only wine')
    parser.add_argument('--gestures', action='store_true', help='Run only gestures')
    ...
    if args.wine and args.gestures:
        logger.error("Can only specify one of '--wine' or '--gestures', not both")
        parser.print_help()
        sys.exit(1)
    ...
    gestures_details = {
            'data': loader.GestureData(verbose=verbose, seed=seed),
            'name': 'gestures',
            'readable_name': 'Gestures',
        }
    if args.wine:
        datasets.append(wine_details)
    elif args.gestures:
        datasets.append(gestures_details)
    elif not args.wine and not args.gestures:
        datasets.append(wine_details)
        datasets.append(gestures_details)
